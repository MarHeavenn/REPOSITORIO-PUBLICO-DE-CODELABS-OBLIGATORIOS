...
img = cv2.imread('comiendoEnClase.jpg')  # reemplaza por tu archivo
...
for _ in range(5):
    t0 = time()
    _ = detector.detect_faces(img_rgb)
    t1 = time()
    times.append(t1 - t0)
avg_time = np.mean(times)
print(f"Tiempo promedio de inferencia por imagen (5 corridas): {avg_time*1000:.1f} ms")
...
thr = 0.95  # Cambiado de 0.90 a 0.95
